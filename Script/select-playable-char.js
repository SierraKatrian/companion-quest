$(document).ready(function(){
        ...
        $.getJSON('./Models/View-Game-Chars.php', function(data){
            ...
            var char = '';
                $.each(data, function(index, gameInfo){
                    char += '<div class="col-sm-2 col-xs-3 character-thumb-container"> <label for="' + gameInfo.role_name + '"> <input id="' + gameInfo.role_name + '" class="character-chk" type="radio" name="availChars" value="' + gameInfo.id + '" /> <img class="' + ((gameInfo.selected == 1) ? "char-disabled" : "character-img") + '" src="Images/' + ((gameInfo.rb_id == 1) ? "apocalypse" : "dungeon") + '-world-characters/' + gameInfo.picture + '" /> <p>The ' + gameInfo.role_name + '</p> </label> </div>';
            });
    ...
    // var radioValue = $("input[name='availChars']:checked");
    // $(radioValue).click(function(){
    //     $('.character-bio-container').show();
    // });
